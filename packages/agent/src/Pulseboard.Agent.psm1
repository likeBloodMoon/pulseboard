...
function Get-SystemMetrics {
    ...
    $diskUsed = $null
    try {
        $fixed = Get-CimInstance Win32_LogicalDisk -Filter "DriveType=3" | Sort-Object Size -Descending | Select-Object -First 1
        if ($fixed) {
            $diskTotal = [math]::Round($fixed.Size / 1GB, 2)
            $diskFree = [math]::Round($fixed.FreeSpace / 1GB, 2)
